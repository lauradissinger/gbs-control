...
void writeProgramArrayNew(const uint8_t* programArray, boolean skipMDSection)
{
  ...
  for (; y < 6; y++)
  {
    ...
    case 1:
      ...
      if (!skipMDSection) {
        loadPresetMdSection();
        if (rto->syncTypeCsync)  GBS::MD_SEL_VGA60::write(0);  // EDTV possible
...
void prepareSyncProcessor() {
  ...
  else if (rto->videoStandardInput <= 4) GBS::SP_DLT_REG::write(0xC0); // old: extended to 0x150 later if mode = 1 or 2
  else if (rto->videoStandardInput <= 6) GBS::SP_DLT_REG::write(0xA0);
  ...
  while (1) {
    ...
    while ((millis() - timeout) < 60) {
      if (GBS::STATUS_SYNC_PROC_HSACT::read() == 1) {
        syncGoodCounter++;
        if (syncGoodCounter >= 60) { break; }
      }
      else if (syncGoodCounter >= 4) { syncGoodCounter -= 3; }
    }
...
float getSourceFieldRate(boolean useSPBus) {
  ...
  if (useSPBus)
  {
    if (rto->syncTypeCsync) {
      ...
      if (testBusSelBackup != 0x0)  GBS::TEST_BUS_SEL::write(0x0);  // RGBHV: TB 0x20 has vsync
    }
...
void doPostPresetLoadSteps() {
    ...
    for (uint8_t i = 0; i < 4; i++) {
      if (GBS::STATUS_INT_SOG_BAD::read() == 1) {
        ...
        if (getVideoMode() == rto->videoStandardInput) {
          ...
          if (rto->videoStandardInput == 1 || rto->videoStandardInput == 3) {
            if (sfr > 58.6f && sfr < 61.4f) ok = 1;
          }
          else if (rto->videoStandardInput == 2 || rto->videoStandardInput == 4) {
            if (sfr > 49.1f && sfr < 51.1f) ok = 1;
          }
  ...
  if (GBS::GBS_OPTION_SCALING_RGBHV::read() == 0) {
    unsigned long timeout = millis();
    while ((!getStatus16SpHsStable()) && (millis() - timeout < 2002)) { delay(4); handleWiFi(0); updateSpDynamic(0); }
    while ((getVideoMode() == 0) && (millis() - timeout < 1505)) { delay(4); handleWiFi(0); updateSpDynamic(0); }
  ...
  SerialM.print(F(" for "));
  if (rto->videoStandardInput == 1)       SerialM.print(F("NTSC 60Hz "));
      ...
      if (rto->syncTypeCsync) {
        ...
        for (int i = 0; i < 16; i++) {
          if (hPeriod == 511 || hPeriod < 200) {
            ...
            if (i == 15) {
              hPeriod = 300; break;             // give up, use short base to get low ignore value later
            }
          }
          else {
            break;
...
void setOutModeHdBypass() {
  ...
  if (rto->videoStandardInput <= 2) {
    ...
    GBS::MD_HS_FLIP::write(1); GBS::MD_VS_FLIP::write(1);
...
void runSyncWatcher()
{
    ...
    rto->videoStandardInput != 15)
  {
    ...
    if (newVideoModeCounter >= 8)
    {
      ...
      for (int a = 0; a < 30; a++) {
        vidModeReadout = getVideoMode();
        if (vidModeReadout == 13) { newVideoModeCounter = 5; } // treat ps2 quasi rgb as stable
        if (vidModeReadout != detectedVideoMode) { newVideoModeCounter = 0; }
      }
      if (newVideoModeCounter != 0) {
        // apply new mode
        SerialM.print(" "); SerialM.print(vidModeReadout);
  ...
  if (rto->videoStandardInput >= 14) { // RGBHV checks
      ...
      // if currently in scaling RGB/HV, check for "SD" < > "EDTV" style source changes
        ...
        if (rto->videoStandardInput != 14) {
          ...
          if (moveOn) {
            ...
            if (rto->videoStandardInput <= 2) { SerialM.println(F(" RGB/HV upscale mode base 15kHz")); }
            else { SerialM.println(F(" RGB/HV upscale mode base 31kHz")); }
    ...
    if ((millis() - lastTimeSogAndPllRateCheck) > 900)
    {
      ...
      if (currentPllRate != 0)
      {
        if (currentPllRate < 1030) { rto->HPLLState = 1; }
        else if (currentPllRate < 2300) { rto->HPLLState = 2; }
        else if (currentPllRate < 3200) { rto->HPLLState = 3; }
        else if (currentPllRate < 3800) { rto->HPLLState = 4; }
        else { rto->HPLLState = 5; }
      }
...
void loop() {
  ...
  if (typeOneCommand != '@') 
  {
    ...
    case '!':
      //fastGetBestHtotal();
      //readEeprom();
      Serial.print(F("sfr: ")); Serial.println(getSourceFieldRate(1));
      Serial.print(F("pll: ")); Serial.println(getPllRate());
    break;
    case '$':
      {
